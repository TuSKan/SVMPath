...
#define SKIP_ELEMENT\
	...
	if(restore_filename)
	{
		...
		errno_t err = fopen_s(&fp_restore, restore_filename,"r");
		if(err)
		{
			fprintf(stderr,"can't open file %s\n", restore_filename);
			exit(1);
		...
		while(fscanf_s(fp_restore,"%d %*f %*f\n",&idx) == 1)
	...
	if(restore_filename)
	{
		...
		{
			if(fscanf_s(fp_restore, "%lf %lf\n", &y_lower, &y_upper) != 2 ||
			   fscanf_s(fp_restore, "%lf %lf\n", &y_min, &y_max) != 2)
		...
		if (fgetc(fp_restore) == 'x') 
		{
			if(fscanf_s(fp_restore, "%lf %lf\n", &lower, &upper) != 2)
				return clean_up(fp_restore, fp, "ERROR: failed to read scaling parameters\n");
			while(fscanf_s(fp_restore,"%d %lf %lf\n",&idx,&fmin,&fmax)==3)
			{
	...
	{
		FILE *fp_save;
		errno_t err = fopen_s(&fp_save, save_filename, "w");
		if(err)
		{
			fprintf(stderr,"can't open file %s\n", save_filename);
			exit(1);
